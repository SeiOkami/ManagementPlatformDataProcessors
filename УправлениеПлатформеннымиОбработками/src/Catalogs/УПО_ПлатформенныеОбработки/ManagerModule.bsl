...
// 
// Параметры:
//  ОписаниеДанных - СтрокаТаблицыЗначений: см. ЭталонныеДанные - Описание эталонного элемента
//
...
Процедура СоздатьЭталонныйЭлемент(ОписаниеДанных)
	...
	ИсключаемыеСвойства = НСтр("ru='Description';en='Наименование'");
	ЗаполнитьЗначенияСвойств(ТекущийОбъект, ОписаниеДанных, , ИсключаемыеСвойства);
...
// 
// Параметры:
//  Ссылка - СправочникСсылка.УПО_ПлатформенныеОбработки
// 
...
Функция СсылкаСуществует(Ссылка)
	...
	|ИЗ
	|	Справочник.УПО_ПлатформенныеОбработки КАК Обработки
	|ГДЕ
