...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Запись.Владелец) 
		ИЛИ Запись.Владелец = УПО_ВызовСервера.ИдентификаторТекущегоПользователяИБ() Тогда
		
		УПО_Клиент.СпроситьИПодключитьВерсиюОбработкиДляТекущегоСеанса(Запись.ВерсияОбработки);
...
Процедура УстановитьНовогоВладельца(Знач ВыбранноеЗначение)
	...
	ИначеЕсли ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		Идентификатор = УПО_КлиентСервер.МультиязычноеСвойствоОбъекта(
...
Процедура ОбновитьПредставлениеПользователя()
	...
	ДанныеПредставления = УПО_Сервер.ПредставлениеПользователяИБ(Запись.Владелец);
...
Функция ИмяФормыВыбораПользователя()
	...
	Возврат Метаданные.РегистрыСведений.УПО_ПодменаПлатформенныхОбработок.Формы.ФормаВыбораПользователяИБ.ПолноеИмя();
