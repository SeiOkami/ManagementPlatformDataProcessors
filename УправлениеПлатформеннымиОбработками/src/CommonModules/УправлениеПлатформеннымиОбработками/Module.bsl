...
Процедура ПодменитьПлатформенныеОбработкиПередЗапускомСистемы() Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка	= РезультатЗапроса.Выбрать(); //ВыборкаИзРезультатаЗапроса: см. ПодменяемыеПлатформенныеОбработкиПередЗапускомСистемы
		Пока Выборка.Следующий() Цикл
...
// 
// Возвращаемое значение:
//  Структура - Представление пользователя ИБ:
// * Текст - Строка
// * ЦветТекста - Цвет 
...
Процедура СписокПодменыПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки) Экспорт
	...
	Для Каждого КлючИЗначение Из Строки Цикл
		...
		Если Строка.Данные.Свойство("Владелец", ИдентификаторПользователя)
			...
			И Строка.Данные.Свойство("Владелец") Тогда
			
			ДанныеПредставления = ПредставлениеПользователяИБ(ИдентификаторПользователя);
			
			Оформление = Строка.Оформление.Получить("Владелец");
...
Функция КоличествоВерсийПлатформыВПлатформенныхОбработках() Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВерсияПриложения) ИЗ Справочник.ВерсииПлатформенныхОбработок";
	РезультатЗапроса = Запрос.Выполнить();
	Если НЕ РезультатЗапроса.Пустой() Тогда
		Выборка = РезультатЗапроса.Выбрать();
		Выборка.Следующий();
